...
namespace TravelBackend.Services
{
    public class UserService
    {
        ...
        public async Task<UserDto?> AdminAuthenticateAsync(string email, string password)
        {
            var user = await _userRepository.GetByEmailAsync(email);

            if (user == null || !user.IsActive || user.UserRole != "admin")
                return null;
        ...
        public async Task AdminCreateUserAsync(string fullName, string email, string password, string userRole, string? nationality, string? preferredLang, string? avatarUrl)
        {
            if (await _userRepository.IsEmailExistsAsync(email))
                throw new Exception("Email already exists.");
            ...
            var user = new Users
            {
                ...
                PreferredLang = preferredLang,
                AvatarUrl = avatarUrl,
                UserRole = userRole, // set the role as given
                IsActive = true,
                CreatedAt = DateTime.UtcNow
            };
